...
def structs(template, study):
    ...
    return [
        ...
        (
            ('case', 'E_GEOM_ALPHA'),
            r'$ E_g $',
            template+' poly CN1'+r' $E_g = \sum_{c \in C} |\Omega_c||\alpha_c(t_e) - \alpha_c(t_0)|$',
            study+'_Eg'
        ),
        (
            ('case', 'E_VOL_ALPHA_REL'),
            r'$ E_v $',
            template+' poly CN1'+r' $E_v(t)  = \frac{\left|\sum_{c \in C}\alpha_c(t)|\Omega_c| - \sum_{c \in C} \alpha_c(t_0)|\Omega_c|\right|}{\left|\sum_{c \in C} \alpha_c(t_0)|\Omega_c|\right|}$',
            study+'_Ev'
        ),
    ]
...
def main():
    ...
    # plot_database.timeplot(study_df, mystructs, savedir)
    # # plot_database.nsmallest_table(study_df, structs, savedir)
    # # plot_database.convergenceplot(study_df, structs, savedir)
