...
def create_pyFoam_listvariationfile(args, meta_name):
    pyFoamVariation_file = meta_name + '.pyFoam-variation'
    run(f"pyFoamRunParameterVariation.py --list-variation {quote(args.casedir)} {quote(args.paramfile)} > {pyFoamVariation_file}",
        shell=True,
        )
    
def create_concrete_casesfile(args, meta_name):
    casesfile = meta_name + '.cases'
...
def main():
    ...
    create_pyFoam_listvariationfile(args, meta_name)
    
    # Creates file which lists all concrete cases
    casesfile = create_concrete_casesfile(args, meta_name)
    ...
    # json_file = casesfile = os.path.join(studydir, studydir + '.json')
    # run(f"case-parameter.py -l {casesfile} convert {pyFoamVariation_file} > {json_file}",
    #     shell=True)
