...
:
    ...
    normals_(mesh_.lookupObject<volVectorField>(surfTensionDict_.get<word>("normals"))),
    alpha_(mesh_.lookupObject<volScalarField>(surfTensionDict_.get<word>("alpha"))),
...
tmp<surfaceScalarField> traceGradGeoNormalSnGradAlpha::faceSurfaceTensionForce() const 
{
    ...
    // TODO(TM): enable the parameterization of the grad scheme. 
    tmp<volVectorField> nPsiTmp = fvc::grad(psi_, "pointCellsLeastSquares");
    volVectorField& nPsi = nPsiTmp.ref();
    nPsi = nPsi / Foam::mag(nPsi);
    ...
    // TODO(TM): enable the parameterization of the gradient scheme. 
    return fvc::interpolate(tr(fvc::grad(nPsi, "pointCellsLeastSquares"))) * 
           fvc::snGrad(alpha_);
}
